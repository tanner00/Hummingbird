	...
	Container({ .Layout = { .SizeX = Fixed(static_cast<float32>(Window->DrawWidth)), .SizeY = Fixed(static_cast<float32>(Window->DrawHeight)) } }, [this]
	{
		...
		[this]
		{
			...
			Container({}, [this]
			{
				...
				Text(StringView { cpuTimeText, Platform::StringLength(cpuTimeText) }, 24.0f, { .Style = { .RGBA = Theme::Text } });

				char gpuTimeText[16] = {};
				Platform::StringPrint("GPU: %.2fms", gpuTimeText, sizeof(gpuTimeText), Renderer->AverageGPUTime * 1000.0);
				Text(StringView { gpuTimeText, Platform::StringLength(gpuTimeText) }, 24.0f, { .Style = { .RGBA = Theme::Text } });
			});
		});
